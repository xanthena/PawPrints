...
Starts the pipeline. It has an `INPUT_MODE` option for choosing input data: `local` reads `jsons/qwen.json` and writes `jsons/final_timeline_qwen.json`, while `cloud` reads `jsons/gemini.json` and writes `jsons/final_timeline_gemini.json`. It is currently hardcoded to `local`.

It can be run either as `python -m event_tracker.main_event_tracker` from the repository root or directly as `python event_tracker/main_event_tracker.py`.
